...
tree* deleteNode(tree *pohon, int nilai) {
    if (pohon == NULL) return NULL;

    if (nilai < pohon->data) {
        pohon->left = deleteNode(pohon->left, nilai);
    } else if (nilai > pohon->data) {
        pohon->right = deleteNode(pohon->right, nilai);
    } else {
        if (pohon->left == NULL && pohon->right == NULL) {
            ...
            cout << "Node bukan leaf, tidak dapat dihapus" << endl;
        }
    }
    return pohon;
}

int countNode(tree *pohon) {
    if (pohon == NULL) return 0;
    return 1 + countNode(pohon->left) + countNode(pohon->right);
}
...
int main() {
    ...
    do {
        ...
        if (pilihan == 1) {
            ...
            cin >> nilai;
            pohon = deleteNode(pohon, nilai);
        } else if (pilihan == 8) {
            cout << "Jumlah node dalam tree: " << countNode(pohon) << endl;
        } else if (pilihan == 0) {
